...
# The program has no need to import time.sleep, since all timed functions use try catch with socket.timeout exception instead
...
def kick(bot_name):     # Method to kick a bot by providing a nickname
    bot_name = bot_name.capitalize()        # Capitalize the input to ensure we find the bot
    if bot_name in nicknames:               # If the bot is connected
        index = nicknames.index(bot_name)
        client = clients[index]             # Finds the correct bot in the client-list
        clients.remove(client)              # Remove bot from list of connections
        nicknames.remove(bot_name)          # Remove nickname from list of nicknames
...
Running = True          # Boolean to let the program know if we want to run the suggestion-loop
Accepting = True        # Boolean to let the program know if we are accepting new connections
thread = threading.Thread(target=acceptLoop)    # Creates a thread to accept new connections
thread.start()                                  # Starts the 'acceptLoop' thread
...
while Running:              # Continues to start new rounds until either Running = False or we use break
    # The loop is looking for new connections and will continue once we have 2 or more
    if len(clients) > 1:    # We need at least 2 connections before starting a round
        ...
        if u_input == "r":          # User wants to send a random suggestion to the clients
            ...
            if not action:
                action = "noth"     # If the user inputs nothing, we suggest [nothing]
...
Accepting = False       # No longer accepting new clients
Running = False         # No longer running the main program
